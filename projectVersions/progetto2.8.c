...
int readInput_tr(){ 
    ...
    if (strcmp(flag, "tr") == 0){
        ...
        tr_Array.list = (tr_list*) malloc (512*sizeof(tr_node*)); //MODIFICATA LA DIMENSIONE DI PARTENZA A 100
        tr_Array.dim = 512;  //MODIFICATA LA DIMENSIONE DI PARTENZA A 100;
        ...
        while(k < config.dim){//computazione della singola configurazione
            ...
            deterministic = 0;
                ...
                if(config.array[k].execute == 1){
                    ...
                    if(deterministic == -1){ //se non trovo tr o accetto o faccio zero
                         //controllo subito di essere in accettazione
               ...
               if(deterministic > 1){  //ho trovato piu transizioni per la configurazione in cui mi trovo ora
                ...
                if(config.array[k].execute == 1){ //determinismo 
