...
typedef struct{ 
    ...
    unsigned long dimension; //paramentro che serve per le realloc.
    int execute;
}configuration;
    ...
    if(strcmp(flag, "run") == 0 && scanfReturn == 1){
        do{
            ...
            if(tape.right != NULL){ //se succede significa che il file è finito, pertanto il programma termina
                ...
                //configFree(); //libera tutta la memoria delle config per questa stringa
...
int moveMachine(){ //l'idea è gestire piu macchine in contemporanea
    ...
    config.array[0].dimension = 6*sizeof(int) + tape.rightDim + tape.leftDim + sizeof(long);
...
char* strStretch(char* string, int rightorleft, int k){ //CONTROLLARE, DA SEG FAULT in certi casi
    ...
    if(rightorleft == 0){ //left
        ...
        for (i = config.array[k].tape.leftDim; i < config.array[k].tape.leftDim + BLANK_SIZE; i++)
            string[i] = '_';
    ...
    if(rightorleft == 1){ //right
        ...
        for (i = config.array[k].tape.rightDim; i < config.array[k].tape.rightDim + BLANK_SIZE; i++)
            string[i] = '_';
