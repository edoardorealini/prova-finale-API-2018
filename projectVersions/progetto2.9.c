...
int readInput_tr(){ 
    ...
    if (strcmp(flag, "tr") == 0){
        ...
        tr_Array.list = (tr_list*) malloc (TR_DIM*sizeof(tr_node*)); //MODIFICATA LA DIMENSIONE DI PARTENZA A 100
        tr_Array.dim = TR_DIM;  //MODIFICATA LA DIMENSIONE DI PARTENZA A 100;
        ...
        do{           
            ...
            if (scanfReturn == 5 && max + 1 > tr_Array.dim) {
                ...
                for(i = tr_Array.dim; i < new_Dim; i++)
        			tr_Array.list[i] = NULL;
                //memset(&tr_Array.list[tr_Array.dim], 0, new_Dim - tr_Array.dim);
...
int moveMachine(){ //l'idea è gestire piu macchine in contemporanea
    ...
    config.array = (configuration*) malloc (CONFIG*sizeof(configuration));
    ...
    do{
        ...
        while(k < config.dim){//computazione della singola configurazione
            ...
            if(config.array[k].execute == 1){
                ...
                if(config.array[k].det == 0){
                    if(config.array[k].rightorleft == 1)
                ...
                if(deterministic == -1){ //zero tr trovate comparibili
                ...
                else{ //ne ho trovata solo 1 (la eseguo da sola)
                    config.array[k].det = 1;
                    moveReturn = moveExec(k);

                    if(moveReturn == -1){   //caso in cui trovo loop e mando subito risultato a U (parziale)
    ...
    if(k < config.dim - 1){
        for(i = 0; i < config.dim && toAdd > 0; i++){ //scorro le esistenti per vedere se c'è spazio
            if(config.array[i].execute == 0){//sono in una configurazione che può essere sovrascritta
                ...
                config.array[i].tape.right = memcpy(config.array[i].tape.right, config.array[k].tape.right, config.array[k].tape.rightDim);
                ...
                config.array[i].tape.left = memcpy(config.array[i].tape.left, config.array[k].tape.left, config.array[k].tape.leftDim);
                ...
                toAdd--;
                createdConfig++;
                //qui devo computare gia la nuova config

                moveReturn = moveExec(i);

                if(moveReturn == -1){   //caso in cui trovo loop e mando subito risultato a U (parziale)
                        config.array[i].execute = 0;
                        uReturns++;
                        free(config.array[i].tape.right); //libero il grosso della memoria occupato dagli array.
...
int moveExec(int k){ //funzione che computa la k-esima configurazione
    ...
    //if(deterministic != 1){ // se deterministic = 1 non serve cercare la tr, è gia nella struttura my_tr
    ...
    //}
    /*else
        returnCode_trLookup = 1; //già trovata da isDeterministic*/
        ...
        switch(config.array[k].rightorleft){    
            case 0: //sono nel lato sx del nastro, uso j    LEFT
                ...
                if(config.array[k].tape.left[j] != my_tr.overwrite)
                    ...
                    if(my_tr.move == 'S'){
                        if(my_tr.current_state == my_tr.next_state && my_tr.input ==  my_tr.overwrite)
                                return -1;
                    }
            break;
...
char* getrightString(){ //la funzione restituisce la stringa con BLANK_SIZE caratteri blank in coda
    ...
    if (scanfReturn != EOF){
        ...
        //memset(&string[len-1], '_', BLANK_SIZE);
...
int isDeterministic(int state, char c){
    ...
    if(state < tr_Array.dim){
	    ...
	    if(counter == 1)
	        return 1; //transizione deterministica già salvata, non serve cercarla ancora
...
char* strStretch(char* string, int rightorleft, int k){ //CONTROLLARE, DA SEG FAULT in certi casi
    ...
    if(rightorleft == 0){ //left
        ...
        for (i = config.array[k].tape.leftDim - 1; i < config.array[k].tape.leftDim - 1 + BLANK_SIZE; i++)
            string[i] = '_';
        //memset(&string[config.array[k].tape.leftDim - 1], '_', BLANK_SIZE);
    ...
    if(rightorleft == 1){ //right
        ...
        //memset(&string[config.array[k].tape.rightDim - 1], '_', BLANK_SIZE);
