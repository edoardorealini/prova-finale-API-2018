...
void moveMachine(){ //l'idea è gestire piu macchine in contemporanea
    ...
    do{
        ...
        moves++;
    }while(moves <= max && accReturn == 0 && computaReturn == 1);    //per ogni ciclo di mossa singola devo provare tutte le configurazioni
...
int computa(int k){ //funzione che computa la k-esima configurazione
    //deterministica!
    int i = 0;
    int j = 0;
    ...
    if(config.array[k].rightorleft == 1){
        ...
        returnCode_trLookup = trLookup(config.array[k].state, config.array[k].tape.right[i]);
    }
        
    if(config.array[k].rightorleft == 0){
        j = config.array[k].index;
        returnCode_trLookup = trLookup(config.array[k].state, config.array[k].tape.left[j]);
    }
